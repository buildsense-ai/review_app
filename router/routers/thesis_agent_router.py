...
async def process_pipeline_async(task_id: str, request: PipelineRequest):
    ...
    try:
        ...
        # 1. 保存纯净的章节JSON文件
        sections_json_path = os.path.join(results_dir, f"thesis_sections_{task_id}_{timestamp}.json")
        with open(sections_json_path, 'w', encoding='utf-8') as f:
            json.dump(unified_sections, f, ensure_ascii=False, indent=2)
        
        # 2. 保存优化后的完整markdown文档
        optimized_md_path = os.path.join(results_dir, f"thesis_optimized_{task_id}_{timestamp}.md")
        
        # 生成优化后的markdown内容
        if corrected_document:
            # 如果有修正文档，使用修正后的内容
            logger.info(f"使用修正后的文档内容，长度: {len(corrected_document)}")
            optimized_content = corrected_document
        else:
            # 如果没有修正，生成基于unified_sections的文档
            logger.info(f"生成基于unified_sections的文档，章节数: {len(unified_sections)}")
            optimized_content = _generate_markdown_from_sections(unified_sections, request.document_content)
        
        # 写入文件
        with open(optimized_md_path, 'w', encoding='utf-8') as f:
            f.write(optimized_content)
        
        logger.info(f"已保存优化后的markdown文档到: {optimized_md_path}")
        logger.info(f"已保存章节JSON到: {sections_json_path}")
        
        # 构建最终结果，包含文件路径信息（用于内部记录）
        final_result = {
            "sections_json_file": sections_json_path,
            "optimized_markdown_file": optimized_md_path,
            "processing_time": processing_time,
        ...
        # 更新任务状态为完成，只返回unified_sections（用户需要的格式）
        update_task_status(task_id, "completed", 100.0, "流水线处理完成", unified_sections)
...
async def get_unified_sections(task_id: str):
    ...
    # 从结果中提取unified_sections
    result = task_info["result"]
    if isinstance(result, dict) and "unified_sections" in result:
        return result["unified_sections"]
    else:
        # 兼容旧格式
        return result
...
async def get_optimized_markdown(task_id: str):
    ...
    if isinstance(result, dict) and "optimized_markdown_file" in result:
        markdown_file_path = result["optimized_markdown_file"]
        try:
